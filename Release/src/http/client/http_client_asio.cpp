...
private:
    ...
    void connect(ExecMode execMode, tcp::resolver::iterator endpoints, ConnectHandler handler)
    {
        ...
        // Try to connect using ipv6
        {
            ...
            if (m_state == ConnectionState::Cancelled)
            {
                l.unlock();
                handler(boost::system::errc::make_error_code(boost::system::errc::operation_canceled), endpoints);
                return;
            }
...
public:
    ...
    virtual ~asio_context()
    {
        ...
        std::static_pointer_cast<asio_client>(m_http_client)->release_connection(m_connection->connection());
    }
